...
const AnimeCard = ({ anime, hero }) => {
  ...
  return (
    <>
      <div onClick={()=>navigate(`/anime/${anime.mal_id}`)} className = {`${hero} relative overflow-hidden rounded-lg shadow transition hover:shadow-lg cursor-pointer`}>
        <img
          alt="anime.title"
        ...
        <div className="relative pt-32 sm:pt-48 lg:pt-64">
          <div className="p-4 sm:p-6 bg-neutral-500/60">
            <div className="flex gap-2">
              {anime.year === null? null : <p className="block badge badge-outline text-xs text-success-content/90">
                {anime.year}
              </p>}
            ...
            <a href="#">
              <h3 className="mt-0.5 text-lg text-accent-content/80 text-left font-bold">{anime.title}</h3>
            </a>
